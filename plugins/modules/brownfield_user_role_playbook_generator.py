...
class UserRolePlaybookGenerator(DnacBase, BrownFieldHelper):
    ...
    def transform_role_resource_types(self, role_details):
        ...
        # Transform resource types into the expected structure
        ...
        for resource in resource_types:
            ...
            if len(parts) >= 2:
                ...
                subcategory = parts[1].lower().replace(" ", "_")
                
                if category not in transformed_permissions:
                    transformed_permissions[category] = [{}]
                
                transformed_permissions[category][0][subcategory] = permission
    ...
    def get_roles(self, network_element, filters):
        ...
        try:
            ...
            if role_filters:
                ...
                # Exclude system default roles for brownfield generation
                final_roles = [role for role in roles if not role.get("name", "").startswith("SUPER-ADMIN") 
                              and not role.get("name", "").startswith("NETWORK-ADMIN")
                              and not role.get("name", "").startswith("OBSERVER")]
